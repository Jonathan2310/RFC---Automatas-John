...
export function validarYMostrarER() {
    var cadena = document.getElementById('cadenaInput2').value;

    if (/^(PEMJ|PEM|PE|P|p|pe|pem|pemj)$/.test(cadena)) {
        document.getElementById('resultado2').innerText = '(' + cadena + ')' + ' Cumple con la expresion regular.';
        document.getElementById('error').innerText = '';
        generarAutomataER(cadena);
    } else {
        document.getElementById('error').innerText = cadena + ' No cumple con la expresion regular.';
...
export function generarAutomataER(cadena) {
    ...
    var svg = d3.select(".box-er").append("svg")
        ...
        .attr("height", 120) // Aumentar la altura para mejorar la visualización
    ...
    var nodes = svg.selectAll("circle")
        ...
        .attr("stroke", "black")
        .attr("stroke-width", 2)
        .attr("fill", "rgba(239, 239, 86, 0.849)");
    ...
    svg.selectAll("text.linkLabel")
        ...
        .attr("y", 70) // Ajustar la posición vertical
        .attr("text-anchor", "middle")
        .attr("alignment-baseline", "middle")
    ...
    svg.append("polygon")
        .attr("points", function() {
            var x = 0; // Ajustar la posición x del triángulo
            var y = 105; // Ajustar la posición y del triángulo
            var size = 30; // Ajustar el tamaño del triángulo
